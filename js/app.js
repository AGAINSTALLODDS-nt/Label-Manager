...
(function() {
        ...
        createLabelElement(label, settings, widthMm, heightMm) {
            ...
            if (!noBarcode) {
                ...
                html += `<div style="margin-bottom:${barcodeOnly ? '2mm' : '1mm'};text-align:center;">
                    <svg id="${bcId}" style="width:100%;height:${bcH}mm;display:block;margin:0 auto;"></svg>
            ...
            // Функция отрисовки штрихкода, вызываемая строго после добавления div в DOM
            div.renderBarcode = function() {
                try {
            ...
            try {
                if (printType === 'thermal') {
                    ...
                    for (let i = 0; i < labels.length; i++) {
                        ...
                        if (typeof labelEl.renderBarcode === 'function') labelEl.renderBarcode();
                    ...
                    for (let startIdx = 0; startIdx < labels.length; startIdx += labelsPerPage) {
                        ...
                        pageLabels.forEach((label, idx) => {
                            ...
                            activeLabels.push(labelEl);
                        });

                        container.appendChild(pageContainer);
                        activeLabels.forEach(el => { if (typeof el.renderBarcode === 'function') el.renderBarcode(); });
    ...
    const App = {
        ...
        bindEvents() {
            ...
            ['print-barcode-format', 'print-center-text', 'print-barcode-only',
        ...
        updatePrintPreview() {
            // ИСПРАВЛЕНО: Привязка к корректному ID контейнера в HTML
            const preview = document.getElementById('print-preview-container');
            if (!preview) return;
            ...
            // Внутренний контейнер для изоляции стилей превью от основной страницы
            const previewInner = document.createElement('div');
            previewInner.style.cssText = 'display:flex; flex-direction:column; align-items:center; width:100%;';

            if (printType === 'thermal') {
                const info = document.createElement('div');
                info.style.cssText = 'margin-bottom:12px;font-size:13px;color:#6B7280; font-weight:500;';
                const orientText = orientation === 'landscape' ? 'альбомная' : 'книжная';
                info.textContent = `🏷️ Термоэтикетка: ${labelWidth}×${labelHeight} мм (${orientText})`;
                previewInner.appendChild(info);

                const labelEl = PDFGenerator.createLabelElement(label, settings, labelWidth, labelHeight);
                labelEl.style.boxShadow = '0 4px 14px rgba(0,0,0,0.15)';
                labelEl.style.border = '1px solid #d1d5db';
                previewInner.appendChild(labelEl);
                preview.appendChild(previewInner);
                
                // ИСПРАВЛЕНО: Безопасная отрисовка штрихкода после добавления в DOM
                if (typeof labelEl.renderBarcode === 'function') labelEl.renderBarcode();
            } else {
                const pageWidth = orientation === 'landscape' ? 297 : 210;
                const pageHeight = orientation === 'landscape' ? 210 : 297;
                ...
                const maxPreviewWidth = preview.clientWidth ? Math.min(preview.clientWidth - 40, 450) : 400;
                ...
                for (let i = 0; i < previewCount; i++) {
                    ...
                    labelEl.style.boxShadow = '0 1px 2px rgba(0,0,0,0.05)';
                    a4Wrapper.appendChild(labelEl);
                    activeElements.push(labelEl);
                }

                previewInner.appendChild(a4Wrapper);
                preview.appendChild(previewInner);
                
                // ИСПРАВЛЕНО: Рендерим штрихкоды во всех миниатюрах только когда они встроены
                activeElements.forEach(el => { if (typeof el.renderBarcode === 'function') el.renderBarcode(); });
            }
        },
