...
unit:'mm',
format: printType === 'thermal' ? [labelWidth, labelHeight] : 'a4',
compress: true
});
...
logging: false,
backgroundColor:'#ffffff',
width: Math.round(pageWidth* 3.78),
...
<button class="action-btn btn-edit" data-id="${label.id}">✏</button>
<button class="action-btn btn-duplicate" data-id="${label.id}">📑</button>
<button class="action-btn delete btn-delete" data-id="${label.id}">🗑</button>
</div></td>
`;
...
=> this.changeQuantity(e.target.dataset.id, 1)));
document.querySelectorAll('.quantity-input').forEach(input=> input.addEventListener('change',(e)=> this.updateQuantity(e.target.dataset.id,
...
=> this.deleteLabel(e.target.dataset.id)));
},
updateBulkActions(){
...
const label= this.labels.find(l=> l.id=== id);
if(label) newLabels.push({...label, id: Utils.generateId(), quantity: 0, createdAt: new
...
dropZone.classList.remove( 'dragover' ); if(e.dataTransfer.files[0])
this.handleQuantityFile(e.dataTransfer.files[0]);});
document.getElementById( 'quantity-btn-cancel').addEventListener( 'click',()=>
{ document.getElementById( 'quantity-import-modal').classList.add( 'hidden' ); this.importQuantityData= null;});
...
tbody.innerHTML= data.slice(0, 10).map(row=>'<tr>'+ headers.map(h=>`<td>$
{Utils.escapeHtml(String(row[h] \|\| ''))}</td>`).join('')+'</tr>').join('');
},
applyQuantityImport(){
...
seller: String(row['Наименование продавца'] \|\| row['Наименование поставщика'] \|\|
row['seller'] \|\| ''),
